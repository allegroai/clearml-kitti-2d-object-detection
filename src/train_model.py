...
def train(model,
    ...
    for i, (img_paths, images, labels) in enumerate(tqdm.tqdm(train_dataloader)):
        ...
        if ((i + 1) % update_gradient_samples == 0) or (i + 1 == len(train_dataloader)):
            optimizer.step()
            optimizer.zero_grad()
            print("update sample", np.transpose(images[0].cpu().numpy(), (1,2,0)).shape)
            Task.current_task().get_logger().report_image(
                title="sample", series="train", iteration=i, image=np.transpose(images[0].cpu().numpy(), (1,2,0)))            
        ...
        Task.current_task().get_logger().report_scalar(title="accuracy", series="recall", iteration=i, value=model.losses["recall"])
        Task.current_task().get_logger().report_scalar(title="accuracy", series="precision", iteration=i, value=model.losses["precision"])
        Task.current_task().get_logger().report_scalar(title="loss", series="train-cls", iteration=i, value=model.losses["cls"])
...
def validation(model,
    ...
    for batch_i, (fnames, images, labels) in enumerate(tqdm.tqdm(valid_dataloader, desc="Detecting objects")):

        images = Variable(images.type(tensor_type))

        with torch.no_grad():
            outputs = model(images)
            outputs = non_max_suppression(outputs, num_classes, conf_thres=conf_thres, nms_thres=0.4)

        for img_i, (output, annotations) in enumerate(zip(outputs, labels)):
            if img_i == 0 and batch_i % 16 == 0:
                path = fnames[img_i]
                print("sample path", path)
                fig = draw_boxes(path, output)
                Task.current_task().get_logger().report_matplotlib_figure(title="detection", series="validation",
                    iteration=batch_i, figure=fig, report_image=True)
...
def train_model(model,
    ...
    for i in range(0, max_epochs):
        ...
        model, mAP = validation(model,
                                ...
                                csv_log_file_valid,
                                tensor_type=torch.cuda.FloatTensor,
                                num_classes=8, conf_thres=val_conf_thre)
        ...
        if mAP > best_mAP:
            ...
            model_path = os.path.join(weights_path, "best_weights_kitti.pth")
            model.save_weights(model_path)
            Task.current_task().update_output_model(model_path, name="best_weights_kitti", comment=f"Epoch {i} mAP={mAP}", auto_delete_file=False)
