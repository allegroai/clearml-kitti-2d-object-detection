...
_colors = [_cmap(i) for i in np.linspace(0, 1, _n_cls_preds)]
_bbox_colors = random.sample(_colors, _n_cls_preds)


def draw_boxes(path, detections, output_path=".", img_i=0, classes=None):
    # TODO - remove this ugly hack
    if not classes:
        # default objects for YOLO 
        base_objects = ["Car", "Van", "Truck", "Pedestrian", "Person_sitting", "Cyclist", "Tram", "Misc"]
        classes = base_objects + [str(i) for i in range(len(base_objects), _n_cls_preds)]
    ...
    if detections is not None:
        ...
        for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
            ...
            color = _bbox_colors[int(np.where(unique_labels == int(cls_pred))[0])]
