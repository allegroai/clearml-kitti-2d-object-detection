...
def main(train_path="../data/train/images/",
         ...
         batch_size=2,
         epochs=30,
         freeze_struct=[True, 5], val_conf_thre=0.8):
    """
    ...
    dataview = DataView()
    dataview.add_query(
        dataset_name='Kitty 2D', version_name='training', roi_query=['Car'])
    dataview.set_labels({
    ...
    dataview.add_query(
        dataset_name='Kitty 2D', version_name='testing')
    dataview.set_labels({
        'Car': 0, 'Van': 1, 'Truck': 2, 'Pedestrian': 3, 'Person_sitting': 4, 'Cyclist': 5, 'Tram': 6, 'Misc': 7
    })
...
if __name__ == "__main__":
    task = Task.init('examples', 'hyper-dataset training yolo kitty', output_uri=True)
    # set default container, so it is easier to replicate (not a must)
    task.set_base_docker(docker_cmd="python:3.8-bullseye", docker_setup_bash_script=["apt install -y libgl1-mesa-glx"])
    # connect arguments so that we could later control them from the UI
    kwargs = dict(
